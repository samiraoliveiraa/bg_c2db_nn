...
# Instancia os trials que serão usados para a criação dos modelos que o optuna testará.
...
def cria_instancia_modelo(trial):
    ...
    num_neuronios = trial.suggest_int("num_neuronios", 30, 600)  
    num_camadas = trial.suggest_int("num_camadas", 3, 5)
    ...
    # Pega a forma que o python entende (que está definida no dicionário) da da função de ativação dada pelo sugest de "funcao_de_ativacao"
...
def funcao_objetivo(
    ...
    # Para rodar no computador normal:
...
# Observando os resultados do optuna:
